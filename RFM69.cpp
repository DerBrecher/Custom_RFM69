...
void RFM69::sendWithoutIRQ(uint16_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK)
{
  writeReg(REG_PACKETCONFIG2, (readReg(REG_PACKETCONFIG2) & 0xFB) | RF_PACKET2_RXRESTART); // avoid RX deadlocks
  uint32_t now = millis();
  while (!canSend() && millis() - now < RF69_CSMA_LIMIT_MS) receiveDone();
  sendFrameWithoutIRQ(toAddress, buffer, bufferSize, requestACK, false);
}
...
// internal function
void RFM69::sendFrameWithoutIRQ(uint16_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK)
{
  //calculate time needed to send based on bitRate and buffer size
  uint32_t bitRate = 0; 
  bitRate = readReg(0x3) <<8;
  bitRate |= readReg(0x4);	
	
  bitRate = 32UL * 1000UL * 1000UL / bitRate;
  uint32_t sendTime = (bufferSize + 13UL) * 8000UL / bitRate;
  
  if (sendTime < 1){ //minimum sendTime is 1
   sendTime = 1;
  }
  ...
  //wait calculated time for transmission to finish
  delay(sendTime); 
